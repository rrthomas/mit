...
\nopagebreak
\begin{description}
\inst{branch}{a-addr —}{If {\tt instruction} is not $0$, raise error $1$. Set {\tt pc} to \spic{a-addr}. Perform the action of {\tt next}.}
...
Since {\tt instruction} must be $0$ when {\tt next} is performed, the rest of an instruction word following {\tt next}, {\tt branch} and {\tt call} must normally be all zero bits.

Non-zero values following {\tt next} and {\tt call} are reserved for the Mit specification; non-zero values following {\tt branch} may be used by implementations to implement extra functionality.
