...
If an invalid or unaligned address is accessed when loading {\tt instruction}, the appropriate error is raised (see section~\ref{errors}).

\begin{description}
\inst{next}{—}{Load the word pointed to by {\tt pc} into {\tt instruction} then add {\tt word\_bytes} to {\tt pc}.}
\end{description}
...
\nopagebreak
\begin{description}
\inst{jump}{a-addr —}{If {\tt instruction} is not $0$, raise error $1$. Set {\tt pc} to \spic{a-addr}. Perform the action of {\tt next}.}
\inst{jumpz}{flag a-addr —}{If \spic{flag} is false then set {\tt pc} to \spic{a-addr} and perform the action of {\tt next}.}
\inst{call}{\aaddr1 — \aaddr2}{If {\tt instruction} is not $0$, raise error $1$. Exchange {\tt pc} with the top stack value. Perform the action of {\tt next}.}
\end{description}


\subsection{Stack manipulation}

These instructions manage the stack (see also section~\ref{stackdepth}):
...
Since {\tt instruction} must be $0$ when {\tt next} is performed, the rest of an instruction word following {\tt next}, {\tt jump} and {\tt call} must normally be all zero bits.
...
\subsubsection{Stack depth manipulation}
\label{stackdepth}

These instructions read and write the {\tt stack\_depth} register:

\nopagebreak
\begin{description}
\inst{get\_stack\_depth}{u —}{Set {\tt stack\_depth}, the number of words on the stack, to \spic{u}.}
\inst{push\_stack\_depth}{— u}{\spic{u} is the value of {\tt stack\_depth}, the number of words on the stack. If there is not enough space for \spic{u} stack items, raise error~$2$.}
\end{description}
