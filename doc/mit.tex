...
\begin{description}
\inst{extra}{{\rm see below}}{Perform extra instruction {\tt ir}; if {\tt ir} is not the opcode of a valid extra instruction, raise error $-1$.}
\end{description}

The following extra instructions are defined:

\begin{description}
\inst{next}{—}{Load the word pointed to by {\tt pc} into {\tt ir}, then add {\tt word\_bytes} to {\tt pc}. If an invalid or unaligned address is accessed when loading {\tt ir}, the appropriate error is raised (see section~\ref{errors}).}
...
These instructions implement unconditional and conditional branches, and subroutine call and return (subroutine return is {\tt jump}):

\nopagebreak
\begin{description}
\inst{nextff}{—}{If {\tt ir} does not have all its bits set, raise error $-1$. Perform the action of {\tt next}.}
\inst{jump}{a-addr —}{If {\tt ir} is not $0$, raise error $-1$. Set {\tt pc} to \spic{a-addr}.}
\inst{jumpz}{flag a-addr —}{If \spic{flag} is false then set {\tt pc} to \spic{a-addr} and set {\tt ir} to $0$.}
\inst{call}{\aaddr1 — \aaddr2}{If {\tt ir} is not $0$, raise error $-1$. Exchange {\tt pc} with the top stack value.}
\end{description}

There are also variants {\tt jumpi}, {\tt jumpzi} and {\tt calli}, which take the address encoded as a {\tt pc}-relative number of words in the remainder of {\tt ir}.
...
\nopagebreak
\begin{description}
\inst{trap}{{\rm undefined}}{Perform arbitrary actions. Note in particular that {\tt trap} may change the stack.}
\end{description}
...
\begin{center}
  \begin{bytefield}[endianness=big,bitwidth=2em]{8}
    ...
    \bitbox{8}{11111111 = {\tt nextff}}
  \end{bytefield}
\end{center}

The operands for {\tt pushi} and {\tt pushreli} are interpreted as signed numbers; the operand of {\tt pushreli} is multiplied by {\tt word\_bytes}. Note that {\tt pushreli}’s immediate constant cannot be $-1$, as the corresponding opcode is used for {\tt nextff}.

\subsection{Instruction opcodes}
\label{opcodes}

Table~\ref{opcodetable} lists the instruction opcodes, and table~\ref{extraopcodetable} the extra instruction opcodes (see section~\ref{extra}). Other instruction opcodes are invalid.
