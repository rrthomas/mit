...
The registers are word quantities; they are listed, with their functions, in table~\ref{regtable}.
...
\inst{set}{\x{u+1}\dots\x0 u — \x0 \x{u}\dots\x1}{Set the \spic{u+1}th stack word to \spic{\x0}, then discard \x0.}
...
\extra{udivmod}{2}{\U1 \U2 — \U3 \U4}{Divide \U1 by \U2, giving the single-word quotient \U3 and the single-word remainder \U4. If \U2 is zero, throw error $-8$.}
\extra{throw}{3}{n —}{Throw error \spic{n}.}
\extra{catch}{4}{\x{u}\dots\x0 \U1 \U2 [\aaddr1] — h \U2 \aaddr2 {\tt ||} \x{u}\dots\x0}{Add an error handler \spic{h} to the current stack frame, then perform the action of {\tt call} (see section~\ref{control}).}
\end{description}
...
Table~\ref{opcodetable} lists the instructions whose least-significant $3$ bits are $000$ or $111$. Other instruction opcodes with those endings are invalid.
