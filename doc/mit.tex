...
``mit'', followed by a zero byte, then the one-byte values of
the {\tt endism} and {\tt word\_bytes} registers of the system which saved the file, then ten zero bytes (adding up to $16$ total), then a word (of the given endianness and size) containing the number of words the code occupies. Then follows the given number of words of code.
