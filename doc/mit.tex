...
\makebox[\stackcom][r]{\tt ( \spic{#3} )}\\[0.5ex]#4}}
...
Alternative stack effects are separated by ``{\tt |}'', and the
circumstances under which each occurs are detailed in the instruction description.
...
\begin{description}
\inst{extra}{{\rm see below}}{Perform extra instruction {\tt ir}; if {\tt ir} is not the code of a valid extra instruction, raise error $-1$.}
\end{description}

The following extra instructions are defined:

\begin{description}
\extra{next}{0}{—}{Load the word pointed to by {\tt pc} into {\tt ir}, then add {\tt word\_bytes} to {\tt pc}. If an invalid or unaligned address is accessed when loading {\tt ir}, the appropriate error is raised (see section~\ref{errors}).}
\extra{halt}{1}{n —}{Raise error $n$.}
\end{description}
...
\nopagebreak
\begin{description}
\inst{trap}{}{Perform trap {\tt ir}; if {\tt ir} is not the code of a valid trap, raise error $-1$.}
\end{description}
...
\nopagebreak
\begin{description}
\inst{jump}{a-addr — {\tt |} —}{If {\tt ir} is $0$, set {\tt pc} to \spic{a-addr}; otherwise, add $\texttt{ir}\times\texttt{word\_bytes}$ to {\tt pc}.}
\inst{jumpz}{flag a-addr — {\tt |} flag —}{If \spic{flag} is false then perform the action of {\tt jump}; otherwise, set {\tt ir} to $0$.}
\inst{call}{\aaddr1 — \aaddr2 {\tt |} — a-addr}{Perform the action of {\tt jump}, then push {\tt pc}’s old value on to the stack.}
\end{description}
...
\opcodetbl{7}{\rm undefined}			{17}{ult}
...
\begin{itemize}
\item Implementations should provide an API to create and run virtual machine instances, and provide access to its registers, stack and memory, and add traps.
