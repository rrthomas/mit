...
\begin{description}
\inst{load}{addr → x}{Push the word stored at \spic{addr}, which must be word-aligned.}
\inst{store}{x addr →}{Store \spic{x} at \spic{addr}, which must be word-aligned.}
\inst{load1}{addr → x}{Push the byte stored at \spic{addr}, setting unused high-order bits to zero, giving \spic{x}.}
\inst{store1}{x addr →}{Store the least-significant byte of \spic{x} at \spic{addr}.}
\inst{load2}{addr → x}{Push the $2$-byte quantity stored at \spic{addr}, setting unused high-order bits to zero, giving \spic{x}. \spic{addr} must be a multiple of $2$.}
\inst{store2}{x addr →}{Store the $2$ least-significant bytes of \spic{x} at \spic{addr}, which must be a multiple of $2$.}
\inst{load4}{addr → x}{Push the $4$-byte quantity stored at \spic{addr}, setting any unused high-order bits to zero, giving \spic{x}. \spic{addr} must be a multiple of $4$.}
...
\begin{description}
\inst{push}{→ x}{Push the word pointed to by {\tt pc} on to the stack, and increment {\tt pc} to point to the following word.}
...
\begin{description}
\inst{call}{\x{\spic{args}}\dots\x1 args rets [addr] → rets ret-addr {\tt ;} \x{\spic{args}}\dots\x1}{Let \spic{ret-addr} be the value of {\tt pc}. Perform the action of {\tt jump}. Move \spic{\x{\spic{args}}\dots\x1} from the caller’s stack to the callee’s.}

\inst{ret}{\pbox[t]{\stackcom}{\qquad\quad rets ret-addr {\tt ;} \x{\spic{rets}}\dots\x1 → \x{\spic{rets}}\dots\x1 \\{\rm or\quad} rets ret-addr {\tt |} \x{\spic{rets}}\dots\x1 → \x{\spic{rets}}\dots\x1 $0$}}{Pop the current computation stack from the call stack; if the call stack is now empty, pop the current call stack. Set {\tt pc} to \spic{ret-addr}, and move \spic{\x{\spic{rets}}\dots\x1} to the new computation stack. Set {\tt ir} to $0$. If a call stack was popped, push $0$ on to the computation stack.}
\end{description}

Error handlers and raising errors:

\begin{description}
\inst{catch}{\x{\spic{args}}\dots\x1 args rets addr → rets ret-addr {\tt |} \x{\spic{args}}\dots\x1}{Set {\tt ir} to $0$. Perform the action of {\tt call}, then push a new call stack on to the catch stack and move the top-most computation stack from the previous call stack to the new one.}

\inst{throw}{rets ret-addr {\tt |} n → n}{Pop the current call stack from the catch stack.}
\end{description}
