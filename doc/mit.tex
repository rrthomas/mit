...
{\tt instruction\_bits} & The number of bits in an instruction. Must be in the range $5$ to $8$ inclusive. \\
 \bottomrule
\end{tabular}
\caption{\label{paramtable}Registers}
\end{center}
\end{table}
...
the {\tt endism} and {\tt word\_bytes} registers of the system which saved the file, then a word (of the given endianness and size) containing the number of words the code occupies. Then follows the code.
